...
def fftWithWindow(FFTData, hannORhamming:str):
    ...
    if hannORhamming == "hann":
        ...
        raise Exception("Window is not/inappropriately specified. Either hann / hamming is right.")
...
FFTedData=FFTonly(zeroPadding(window(trimmedXD, "hann")))
#FFTedData=FFTonly(window(trimmedXD, "hann"))
...
Afsum = absFFTData[0][getIndexOfNearestValue(absFFTData[1],inputFreq1+inputFreq2)]
Afdif = absFFTData[0][getIndexOfNearestValue(absFFTData[1],inputFreq1-inputFreq2)]

af1_0 = source_amp*10**-10
af2_0 = source_amp*10**-10

af1 = Af1*2/int(len(zeroPadding(trimmedXD)))*10**-10
af2 = Af2*2/int(len(zeroPadding(trimmedXD)))*10**-10
asum = Afsum*2/int(len(zeroPadding(trimmedXD)))*10**-10
adif = Afdif*2/int(len(zeroPadding(trimmedXD)))*10**-10

a1s = source_amp*10**-10
a2 = 2*A2/int(len(zeroPadding(trimmedXD)))*10**-10

betaSHG = getBetaSHG(a1s,a2,waveLength1,delta_x*(10**-10))
f = open("betaSHG.txt", "w")
f.write(str(betaSHG))
f.close

betaFreqMix = getBetaFreqMix(asum, adif, af1_0, af2_0, inputFreq1, inputFreq2, delta_x*10**-10, waveVelocity)
f = open("betaFreqMix.txt", "w")
...
if HPC_OR_LOCAL == "LOCAL":
    ...
    ax.plot(absFFTData[1,0:getIndexOfNearestValue(absFFTData[1], max(inputFreq1, inputFreq2))*4], absFFTData[0,0:getIndexOfNearestValue(absFFTData[1], max(inputFreq1, inputFreq2))*4])
    ...
    x_bar = np.array([inputFreq1-inputFreq2, inputFreq2, inputFreq1, inputFreq1+inputFreq2])
    y_bar = np.array([Afdif, Af2, Af1, Afsum])
    x_bar = x_bar / 10**11
    #print(x_bar)
    #print(y_bar)
    plt.bar(x_bar, y_bar)
